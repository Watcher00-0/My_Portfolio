...
const setNavScrolledState = () => nav.classList.toggle('is-scrolled', window.scrollY > 12);
setNavScrolledState();
...
sectionLinks.forEach((link) => link.addEventListener('click', () => {
  ...
  if (section) {
    section.classList.remove('target-highlight');
    requestAnimationFrame(() => section.classList.add('target-highlight'));
  }
}));
...
const navigationTargets = sectionLinks.map((link) => document.querySelector(link.getAttribute('href'))).filter(Boolean);
...
/* Interactive magnetic dot-grid background (dark mode only) */
(() => {
